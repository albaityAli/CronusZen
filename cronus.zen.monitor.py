...
async def check_cronus_zen():
    ...
    if x == "SOLD OUT":
        ...
        await channel.send(#call buy function from checkingOutProperly after creating instances))


    elif x == "TimeoutException":
        channel = await client.fetch_channel(836377623252303883)
...
async def successful_order(ctx):
    ...
    nicely.add_field(name="Name", value=f"{checkingOutProperly.Ali.name_on_card}   ", inline=True)
    nicely.add_field(name="Email", value=checkingOutProperly.Ali.email, inline=True)
